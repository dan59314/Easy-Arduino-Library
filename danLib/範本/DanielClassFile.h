...
#ifndef _DanielClassFile_h
#define _DanielClassFile_h
...
class TDanielClasser
{
private:
  byte FMemers1;

  void(*FOnCallBack_myCheck)(TDanielClasser*, byte &paramVal); /*TDanielClasser　的　Class callback function*/
...
public:
  ...
  TDanielClasser(byte val, void(*pCallbackFunc)(TDanielClasser*, byte &paramVal)=NULL); /* overload constructor pCallBackFunc(sender, paramVal) */

  void Initial(byte val,  void(*pCallbackFunc)(TDanielClasser *, byte &paramVal)=NULL);
  //void HelloWorld(TDanEnum de); // HelloWord (code 950 用雙斜線註解會造成編譯錯誤) 
  void Check_Run(void); /* 執行 Check_Run指令*/
};


extern TDanielClasser DanielClasser;
