  ...
  注意
    ...
    3. 所有資料運作皆以 真實座標系統 ( 右手系統 ) 運作，即
       逆時鐘旋轉時，角度為正， 順時鐘旋轉時角度為負。
		...
		4. 先置換 "IRManager" -> 類別名(EX:M2dManager)", 接著置換 "IRManage" -> 檔案名稱(EX: M2dManage)
-------------------------------------------------------------------------
...
private:
  ...
  void(*FOnCallBack_myCheck)(TIRManager*, byte &paramVal); /*TIRManager　的　Class callback function*/
...
public:
  ...
  TIRManager(byte val, void(*pCallbackFunc)(TIRManager*, byte &paramVal)=NULL); /* overload constructor pCallBackFunc(sender, paramVal) */

  void Initial(byte val,  void(*pCallbackFunc)(TIRManager *, byte &paramVal)=NULL);
  void Check_Run(void); /* 執行 Check_Run指令*/
};


extern TIRManager IRManager;
