...
private:
  ...
  void(*FOnCallBack_myCheck)(TStringManager*, byte &irCode); /*TStringManager　的　Class callback function*/
...
public:
  ...
  TStringManager(PByte pActionStart0, void(*pCallbackFunc)(TStringManager*, byte &irCode)); /* overload constructor pCallBackFunc(sender, irCode) */

  void Initial(PByte pRunStart0,  void(*pCallbackFunc)(TStringManager *, byte &irCode));
  void Run(void); /* 執行 Run指令*/
    
  int Pos(String subStr, String allStr);
  bool Split(String sInput, const char *sDiv, String sCommands[], int cmdLength, byte &cmdNum);
  bool Split(String sInput, const char *sDiv, String sCommands[], byte &cmdNum);
  bool Get_NextString(String &sMix, String sDivide, int &startID, String &subStr, bool JumpToNextFieldStart=true);


};


extern TStringManager StringManager;
