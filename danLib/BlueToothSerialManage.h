...
// BlueToothSerialManage.h

#ifndef _BlueToothSerialManage_h
#define _BlueToothSerialManage_h
...
#define BaudRate_CH05 38400  // Android BlueTooth BaudRate
#define BaudRate_BLE4 9600	 // BlueToothLE BaudRate



class TBlueToothSerialManager
{
private:
  String FLineStr;
  long FBaudRate=38400;

  void(*FOnProcessString)(String &str); /*TBlueToothSerialManager　的　Class callback function*/
...
protected:
  ...
  void DoProcessString(String &str);
  bool Check_And_Process_String(String &lnStr);
public:

  TBlueToothSerialManager();
  ~TBlueToothSerialManager();

  TBlueToothSerialManager(long BaudRate=38400, void(*pProcessString)(String &str)=NULL); /* overload constructor pProcessString(sender, irCode) */

  void begin(long BaudRate=38400, void(*pProcessString)(String &str)=NULL );
  //void HelloWorld(TDanEnum de); //HelloWord (code 950 用雙斜線註解會造成編譯錯誤)
  bool Receive_CompleteString(String &str, bool &blHasInput);
  bool Check_And_Process_String();
  void Send_String(String &str); /* 執行 Run指令*/
  void Send_ByteArray(byte *byteAry, int aLength);
...
extern SoftwareSerial gBlueToothSerial; //在 cpp 初始化 SoftwareSerial gBlueToothSerial(pinRx, pinTx); //10, 11); // RX, TX 

extern TBlueToothSerialManager BlueToothSerialManager;
